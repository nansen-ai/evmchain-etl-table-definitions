...
-- Block by block balance of Liquidity Mining Power

with bpt_transfers as (
    select * from `blockchain-etl.arbitrum_balancer.view_LM_transfers`
),
...
calendar AS (
    select number as block_number from `nansen-datasets-prod.crypto_arbitrum.blocks`
),
running_balances as (
